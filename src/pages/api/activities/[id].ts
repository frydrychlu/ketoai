...
import { getDailyExpenditure } from "@/lib/services/activities";
...
export const DELETE: APIRoute = async (context) => {
  ...
  const deleteResult = await supabase.from("activities").delete().eq("id", id).select().single();

  if (deleteResult.error || !deleteResult.data) {
    return Response.json({ error: "Activity not found" }, { status: 404 });
  }

  const deleted = deleteResult.data as Activity;
  const total = await getDailyExpenditure(supabase, deleted.day);
